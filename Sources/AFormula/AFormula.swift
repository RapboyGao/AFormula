...
extension AFormula {
    public static func point(x: Double, y: Double) -> AFormula {
        return .value(.point(x: x, y: y))
    }

    public static func location(latitude: Double, longitude: Double) -> AFormula {
        return .value(.location(latitude: latitude, longitude: longitude))
    }

    public static func boolean(_ value: Bool) -> AFormula {
        return .value(.boolean(value))
    }

    public static func groundWind(limit: AWindLimit) -> AFormula {
        return .value(.groundWind(limit: limit))
    }

    public static func minutes(_ value: Int) -> AFormula {
        return .value(.minutes(value))
    }

    public static func minutes(h hours: Int, m minutes: Int) -> AFormula {
        return .value(.minutes(hours * 60 + minutes))
    }

    public static func calendar(_ date: Date) -> AFormula {
        return .value(.calendar(date, timeZone: .current))
    }

    public static func dateDifference(_ components: DateComponents) -> AFormula {
        return .value(.dateDifference(components))
    }

    public static func p(_ formula: AFormula) -> AFormula {
        .parenthesis(formula)
    }

    public static func f(_ function: AFunction, args: [AFormula]) -> AFormula {
        .function(id: function.id, args: args)
    }
}

extension AFormula {
    public static func + (left: AFormula, right: AFormula) -> AFormula {
    ...
    public static prefix func - (formula: AFormula) -> AFormula {
...
extension AFormula {
    ...
    public init(tokens: [AToken]) throws {
